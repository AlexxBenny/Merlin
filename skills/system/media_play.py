...
"""
State-aware media PLAY skill.

Reads WorldSnapshot.state.media to determine current playback state.
Only sends the play/pause toggle key if media is NOT already playing.
Idempotent: calling "play" when already playing is a no-op, not a pause.
"""
...
class MediaPlaySkill(Skill):
    """
    Resume/start media playback — state-guarded.

    Reads snapshot.state.media to determine current state:
    - If no media session detected → no-op (reason: no_media_session)
    - If already playing → no-op (reason: already_playing)
    - If paused → sends VK_MEDIA_PLAY_PAUSE toggle to resume
    ...
    contract = SkillContract(
        ...
        intent_verbs=["play", "start", "resume", "begin"],
        intent_keywords=["music", "song", "media", "track", "audio"],
        domain="system",
    ...
    def execute(self, inputs: Dict[str, Any], world: WorldTimeline, snapshot=None) -> SkillResult:
        ...
        if media.is_playing:
            return SkillResult(
                outputs={"changed": False},
                metadata={"domain": "system", "reason": "already_playing"},
            )
